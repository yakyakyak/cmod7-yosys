...
run_docker() {
    docker run --rm \
        -v /home/yak/Projects/Claude/cmod7-yosys:/work \
        -w /work \
        -u "$(id -u):$(id -g)" \
...
run_docker "yosys -p 'read_verilog library/uart/uart_rx.v library/uart/uart_tx.v src/pwm_generator.v src/reg_ctrl.v platforms/cmod_a7/top.v; synth_xilinx -flatten -abc9 -arch xc7 -top top; write_json ${BUILD_DIR}/${PROJECT}.json'"
...
run_docker "nextpnr-xilinx --json ${BUILD_DIR}/${PROJECT}.json --xdc platforms/cmod_a7/cmod_a7.xdc --chipdb ${BUILD_DIR}/${PART}.bin --fasm ${BUILD_DIR}/${PROJECT}.fasm"
